...
object InteractionPositionValidation extends EventValidation {
  case class NoRankingEventsError(msg: String) extends ValidationError
  ...
  override def validate(config: Config, events: List[Event]): List[EventValidation.ValidationError] = {
    ...
    rankings.values.map(_.size).maxOption match {
      case None =>
        logger.warn(s"$name = FAIL (no ranking events found in the dataset)")
        List(NoRankingEventsError("no ranking events found"))
