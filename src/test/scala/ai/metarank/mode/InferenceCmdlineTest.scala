...
class InferenceCmdlineTest extends AnyFlatSpec with Matchers {
  it should "parse explicit options" in {
    val result = InferenceCmdline
      ...
      .unsafeRunSync()
    result shouldBe InferenceCmdline(model = "foo", config = "foo", format = JsonCodec, savepoint = "foo")
  }
  ...
  it should "parse options from env" in {
    val result = InferenceCmdline
      ...
      .unsafeRunSync()
    result shouldBe InferenceCmdline(model = "foo", config = "foo", format = JsonCodec, savepoint = "bar")
  }
