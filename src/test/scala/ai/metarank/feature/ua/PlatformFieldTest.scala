...
class PlatformFieldTest extends AnyFlatSpec with Matchers with TableDrivenPropertyChecks {
  ...
  val values = Table(
    ...
    ("Mozilla/4.0 (compatible; MSIE 9.0; Windows NT 6.1)", Some("desktop")),
    (
  ...
  it should "parse user agent platforms" in {
    forAll(values) { (ua: String, platform: Option[String]) =>
      PlatformField.value(parser.parse(ua)) shouldBe platform
    }
  }
}
