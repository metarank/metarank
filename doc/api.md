...
- [Flush API](#flush) finalizes all buffered click-throughs on demand.
...
Note that rankings and interactions are joined into click-throughs in a session buffer: a click-through is finalized
(written to the training store, with synthetic `impression` events emitted per the
[cascade click model](click-models.md#cascade-model-and-click-through-rate)) when its session expires after
`core.clickthrough.maxSessionLength` (30 minutes by default) of inactivity - not at the end of each `/feedback` request.
To finalize buffered click-throughs immediately, use the [Flush API](#flush).
...
Finalizes all click-throughs currently waiting in the session buffer without waiting for the
`core.clickthrough.maxSessionLength` timeout: they are written to the training store and synthetic `impression`
events are emitted. Useful for demos, integration tests and batch senders which know that no more interactions
are coming.

Keep in mind that `/flush` finalizes **all** open sessions: an interaction arriving afterwards for an
already-flushed ranking will form a degenerate single-item click-through, so only call it when the sessions
you care about are complete.

### Response

The same JSON message as the [Feedback API](#feedback) returns:
```json
