...
case class NumberFeature(schema: NumberFeatureSchema) extends MFeature[NumberFeatureSchema] {
  ...
  override def writes(event: Event): Iterable[Put] = for {
    ...
    Put(
      Key(conf, Tenant(event.tenant.getOrElse("default")), meta.item.value),
      meta.timestamp,
  ...
  override def keys(request: Event.ImpressionEvent): Traversable[Key] =
    request.items.map(item => Key(conf, Tenant(request.tenant.getOrElse("default")), item.id.value))

  override def value(request: Event.ImpressionEvent, state: Map[Key, FeatureValue], id: String): MValue =
    state.get(Key(conf, Tenant(request.tenant.getOrElse("default")), id)) match {
