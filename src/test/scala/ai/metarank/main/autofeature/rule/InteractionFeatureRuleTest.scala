...
import ai.metarank.main.command.autofeature.rules.{StringFeatureRule, InteractionFeatureRule}
...
class InteractionFeatureRuleTest extends AnyFlatSpec with Matchers {
  it should "generate interacted_with for cats and ints" in {
    val result = InteractionFeatureRule.make("click", "good")
