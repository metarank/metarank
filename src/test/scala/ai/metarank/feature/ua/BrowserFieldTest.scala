...
class BrowserFieldTest extends AnyFlatSpec with Matchers with TableDrivenPropertyChecks {
  ...
  val values = Table(
    ...
    ("Mozilla/4.0 (compatible; MSIE 9.0; Windows NT 6.1)", Some("ie")),
    (
  ...
  it should "parse user agent browsers" in {
    forAll(values) { (ua: String, value: Option[String]) =>
      BrowserField.value(parser.parse(ua)) shouldBe value
    }
  }
